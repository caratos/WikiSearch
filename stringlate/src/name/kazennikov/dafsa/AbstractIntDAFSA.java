...
public abstract class AbstractIntDAFSA extends IntDaciukAlgoIndexed {
	...
	/**
	 * Basic FSA state. Doesn't store the final value. The value is stored in the
	 * DAFSA itself
	 * 
	 * @author Anton Kazennikov
	 *
	...
	public void toDot(PrintWriter pw) throws IOException {
		...
		pw.println("#total of states=" + states.size());
		...
		for (State n : states) {
			...
			for (int i = 0; i < n.outbound(); i++) {
				pw.printf("%d -> %d [label=\"%s\"];%n", n.number, decodeDest(n.next.get(i)),
						"" + ((char) decodeLabel(n.next.get(i))));
			}
