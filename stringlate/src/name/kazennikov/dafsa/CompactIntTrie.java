...
public class CompactIntTrie {
	...
	/**
	 * Chunked memory allocator. Splits given memory area into chunks nd tracks
	 ...
	 * Add new empty state
	 * 
	 * @param reserve should we reserve memory for a transition? true on known
	 *                non-leaf states
	 * @return state index
	 */
	...
	/**
	 *
	 * Add word to the trie. It is the caller responsibility to establish such
	 * protocol that guarantee that the final state couldn't be relocated (i.e. that
	 * the given word couldn't be a prefix of some other word)
	 *
	 * @param l word to add
	...
	protected int addSuffix(TIntList l, int pos, int prevState, int state) {
		while (pos < l.size()) {
			...
			state = addTransition(prevState, state, l.get(pos), next); // reduce relocations by reserving transitions
																		// for non-last states
			prevState = state;
			state = next;
			pos++;
	...
	public void sortTransitions(IntComparator c) {
		...
		TIntDeque q = new TIntDeque(start);
		q.add(start);
