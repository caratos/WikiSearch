...
/**
 * Generic algorithm for constructing minimal DAFSA (deterministic acyclic
 * finite state automata) or minimial tries.
 * 
...
public abstract class IntDaciukAlgoIndexed {
	...
	/**
	 * Adds a transition from src to dest on given label, if a transition from src
	 * with given label already exists, then change it to new destination state.
	 * 
	 ...
	 * 
	 * @return true, if state has changed, else false (this is possible then state
	 *         is already final)
	 */
	 ...
	 * 
	 * @return number of registered state, or -1 (INVALID_STATE) if no such state
	 *         exist
	 */
	...
	protected TIntList addSuffix(TIntList states, int s, TIntList seq, int start, int end) {
		...
		// this check is needed only when we set finalty on already existing state (not
		// fresh created one)
		if (start == end) {
			if (!hasFinal(current))
				regRemove(current);
	...
	TIntList commonPrefix(TIntList seq) {
		...
		for (int i = 0; i != seq.size(); i++) {
			...
			// Carlos says: I modified this code to get unique entries for my project
			if (in == 47 && next != Constants.INVALID_STATE) {
				return null; // This is the only case
			}
	...
	public void addMinWord(TIntList seq) {
		/*
		 * 1. get common prefix 2. find first confluence state in the common prefix 3.
		 * if any, clone it and all states after it in common prefix 4. add suffix 5.
		 * minimize(replaceOrRegister from the last state toward the first)
		 */
		TIntList stateList = commonPrefix(seq);

		// Carlos says: This is the only one case
		if (stateList == null)
			return;
		...
		/*
		 * index of stop for replaceOrRegister a pointer to the state before
		 * modifications caused by this word addition.
		 * 
		 * The logic is: if the state isn't changed by replaceOrRegister we can safely
		 * bail out as all states before this won't change either
		 */
		int stopIdx = confIdx == -1 ? stateList.size() : confIdx;

		if (confIdx > -1) {
			int idx = confIdx;
			regRemove(stateList.get(idx - 1)); // as we will clone confluence state and change previous to link the
												// cloned
